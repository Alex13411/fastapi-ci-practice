...
class RecipeCreate(BaseModel):
    title: str = Field(
        ..., max_length=100, description="Название блюда", examples=["Борщ"]
    )
    cooking_time: int = Field(
        ..., gt=0, description="Время приготовления в минутах", examples=[60]
    )
    ingredients: List[str] = Field(
        ..., description="Список ингредиентов", examples=[["Свекла", "Капуста", "Мясо"]]
    )
    description: str = Field(
        ...,
        max_length=1000,
        description="Пошаговое описание рецепта",
        examples=["Сварите бульон..."],
    )
